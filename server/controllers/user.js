...
export const updateUser = async (req, res) => {
  const { profileImg, bio, firstName, lastName, oldPassword, newPassword } =
    req.body;
  const { personal } = req.query;
  try {
    const user = await User.findOne({ where: { uuid: req.params.id } });
    if (!user)
      return res.status(StatusCodes.NOT_FOUND).json({ msg: "user_not_found" });
      ...
      if (oldPassword) {
        ...
        if (!isPasswordCorrect)
          return res
            ...
            .json({ oldPassword: "wrong_password" });
...
export const getUserRequests = async (req, res) => {
  try {
    const requests = await Request.findAll({
      where: { userId: req.userId, status: { [Op.not]: "pending" } },
  ...
  try {
    ...
    if (!email) {
      ...
      return res
        ...
        .json({ msg: "2factor_verification_enabled" });
    } else {
      const notifications = await Notification.findAll({
      ...
      return res
        ...
        .status(StatusCodes.OK)
        .json({ user, notifications, requests });
    }
  } catch (err) {
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(err);
  }
