...
const Review = ({ review }) => {
  ...
  const {
    ...
    averageRating,
    createdAt,
    user: { firstName, lastName, profileImg, uuid: userId, profUpdated },
  } = review;
  return (
    <Card>
      ...
      <Card.Body>
        ...
        <p className="mt-2">
          <strong>{t("reviewed_piece")}</strong> {reviewedPiece}{" "}
        </p>
        <p className="mt-2">
          <strong>{t("grade")}</strong> {grade}
        </p>
        <p className="mt-2 d-flex align-items-center gap-3">
          <strong>{t("average_rating")}</strong>{" "}
          <Rating allowFraction initialValue={averageRating} readonly />
        </p>
      </Card.Body>
      <Link to={`/profile/${userId}`} className="link">
        <Card.Footer className="d-flex justify-content-around align-items-center">
          ...
          <div className="right">
            <p className="mt-2 ">
              {t("created_at")}: {moment(createdAt).format("L")}
            </p>
          </div>
        </Card.Footer>
      </Link>
      <Link className="link" to={`/review/${uuid}`}>
        <Button className="view">{t("view_more")}</Button>
      </Link>
    </Card>
  );
};
