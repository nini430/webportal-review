...
const CreateReview = ({ update }) => {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      const { data } = await axiosFetch.get("/reviews/tags", {
    ...
    const fetchUsers = async () => {
      const { data } = await axiosFetch.get("/admin/allusers/?role=user", {
        withCredentials: true,
      });
      return data;
    };
  ...
  useEffect(() => {
    ...
    if (update) {
      fetchReview().then((data) => {
        ...
        setImages(
          data.review.reviewImages
            .filter((item) => !item.isDefault)
            .map((item) => ({ name: item.id, img: item.img, uploaded: true }))
        );
      });
    }
  }, [update, id]);
  ...
  const updateMutation = useMutation(
    async (review) => {
      ...
      return axiosFetch.put(
        `/reviews/edit/${id}`,
        { ...review, addedImages: imgs, deletedImages: deletedImgs },
        { withCredentials: true }
      );
    },
    {
    ...
    <div className="create">
      <Form onSubmit={(e) => e.preventDefault()} className="form">
        ...
        <Form.Group className="mb-2">
          ...
          <Form.Control
            isInvalid={errors.reviewedPiece}
            value={values.reviewedPiece}
            name="reviewedPiece"
            onChange={handleChange}
            type="text"
          />
          {errors.reviewedPiece && (
            <p className="error">{errors.reviewedPiece}</p>
          )}
        </Form.Group>
        <Form.Group className="mb-2">
              ...
              >
                {images.map((img) => (
                  <div className="img">
                    ...
                    <div
                      onClick={(e) =>
                        cancelHandler(img.img, img.name, img.uploaded, e)
                      }
                      className="cancel"
                    >
                      X
        ...
        {!update && currentUser.role === "admin" && (
          <Form.Group className="mb-2">
            <Form.Label>{t("created")}</Form.Label>
            <Typeahead
              multiple={false}
              id="users"
              options={users?.map((user) => user.uuid)}
